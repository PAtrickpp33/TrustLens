...
/** Story-led ScamHub dashboard with your original charts */
...
export default function LandingDashboard() {
  ...
  const years = useMemo(() => ["All", ...Array.from(new Set(rows.map(r=>String(r.Year)))).sort()], [rows]);
  const categories = useMemo(() => ["All", ...Array.from(new Set(rows.map(r=>(r.Category||"").trim()).filter(Boolean))).sort()], [rows]);

  const filtered = useMemo(
    () => rows.filter(r => (year==="All" || String(r.Year)===year) && (category==="All" || (r.Category||"")===category)),
    [rows, year, category]
  );
  ...
  const byCategory = useMemo(() => {
    ...
    filtered.forEach(r=>{
      ...
      const v=m.get(k)||{loss:0,reports:0}; v.loss+=r.LossMillions; v.reports+=r.Reports; m.set(k,v);
    });
  ...
  return (
    <section style={{ padding: "2rem 1rem 3rem", maxWidth: 1200, margin: "0 auto" }}>
      {/* HERO */}
      <div style={{background:"linear-gradient(135deg, #0ea5e9 0%, #2563eb 60%, #4f46e5 100%)",color:"#fff",borderRadius:18,padding:"22px 18px",boxShadow:"0 12px 24px rgba(0,0,0,.15)",marginBottom:16}}>
        <h1 style={{margin:0,fontSize:"1.9rem",fontWeight:800}}>ScamHub — Learn • Check • Act</h1>
        <p style={{margin:"8px 0 0",maxWidth:820,opacity:0.95}}>
          A story of how scams impact Australians. Explore trends, the riskiest categories, and common red flags — then test yourself.
        </p>
      </div>

      {/* INSIGHT STRIP */}
      <div style={{background:"#EFF6FF",border:"1px solid #BFDBFE",borderRadius:12,padding:"10px 14px",marginBottom:18,color:"#334155"}}>
      ...
      {/* CTA */}
      <div style={{background:"#fff",border:`1px solid ${COLORS.border}`,borderRadius:18,padding:18,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12, marginTop:16}}>
        <div>
          <h3 style={{ margin:0, fontWeight:800, color:"#1E3A8A" }}>Test yourself: “Could this be a scam?”</h3>
          <p style={{ margin:"6px 0 0", color:COLORS.subtext }}>
            Take our interactive quiz across job, email, SMS and investment scams. Instant explanations + sources.
          </p>
        </div>
        <Link to="/features" style={{background:"#1D4ED8",color:"#fff",padding:"10px 16px",borderRadius:10,textDecoration:"none",fontWeight:700,boxShadow:"0 8px 16px rgba(29,78,216,.25)"}}>
          Start Quiz
        </Link>
      </div>
    </section>
...
function Section({title, blurb, children}:{title:string; blurb?:string; children:React.ReactNode}) {
  return (
    <section style={{ margin: "18px 0 22px" }}>
      <h2 style={{ margin: 0, color: "#1E3A8A", fontWeight: 800, fontSize: "1.25rem" }}>{title}</h2>
      {blurb && <p style={{ margin: "6px 0 12px", color: COLORS.subtext }}>{blurb}</p>}
      {children}
    </section>
  );
}
function TipsBanner() {
  return (
    <div style={{background: COLORS.bgSoft,border:`1px solid ${COLORS.border}`,borderRadius:16,padding:16,margin:"6px 0 18px"}}>
      <h3 style={{ margin:0, color:"#065F46", fontWeight:800 }}>Quick safety check (10 sec)</h3>
      <ul style={{ margin:"8px 0 0 18px", color:COLORS.text }}>
        <li><strong>Hover/long-press links</strong> — look for look-alike domains.</li>
        <li><strong>Watch for urgency</strong> — “act now”, “verify in 24h”.</li>
        <li><strong>Confirm payments off-thread</strong> via official contacts.</li>
      </ul>
      <p style={{ margin:"8px 0 0", color:COLORS.subtext, fontSize:12 }}>Guidance only — if unsure, don’t click.</p>
    </div>
  );
}
...
function Select({ label, value, onChange, options }:{label:string; value:string; onChange:(v:string)=>void; options:string[]}) {
  return (
    <label style={{display:"flex", gap:8, alignItems:"center", color:COLORS.text, fontSize:13}}>
      <span>{label}:</span>
      <select value={value} onChange={(e)=>onChange(e.target.value)} style={{ border:`1px solid ${COLORS.border}`, borderRadius:10, padding:"6px 10px", background:"#fff", color:COLORS.text }}>

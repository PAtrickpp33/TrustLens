 ...
 * - Theme for Overview (no Tailwind)
 * - Mock data
 */
...
const THEME = {
  ...
  // charts
  lineA: "#06b6d4",   // scans
  lineB: "#fb923c",   // detected
  donut: ["#a855f7","#06b6d4","#22c55e","#fb923c","#ef4444","#8b5cf6"],
};
...
export default function LandingOverview() {
  ...
  return (
    <section style={{padding:'1.5rem', maxWidth: 1200, margin:'0 auto', color:THEME.text, background: THEME.bg}}>
      {/* Header */}
        ...
        <div style={{display:'inline-flex', gap:8}}>
          {[7,30,90].map(r=>(
            <button key={r}
              ...
              style={{
                border:`1px solid ${THEME.border}`, borderRadius:12, padding:'6px 10px', fontSize:12, cursor:'pointer',
                background: r===range? THEME.primarySoft:'#fff', color: r===range? THEME.primaryInk:THEME.text
              }}>
              {r} Days
            </button>
          ))}
...
/* ---------------- UI primitives ---------------- */
function Kpi({ title, value, note, gradient, fg }:{
  title:string; value:string; note?:string; gradient:string; fg:string;
}) {
  return (
    <div style={{
      background: gradient, color: fg, borderRadius:14, padding:'14px 12px',
      textAlign:'center', boxShadow:'0 6px 16px rgba(2,0,40,0.08)', border:`1px solid ${THEME.border}`
    }}>
      ...
      {note && <div style={{fontSize:11, color:THEME.heading, opacity:.8}}>{note}</div>}
    </div>
  );
}
...
function Card({id, title, subtitle, children}:{id?:string; title:string; subtitle?:string; children:React.ReactNode}) {
  return (
    <div id={id} style={{
      background:THEME.cardBg, border:`1px solid ${THEME.border}`, borderRadius:16, padding:16,
      boxShadow:'0 2px 10px rgba(0,0,0,0.05)'
    }}>
      <h3 style={{margin:0, color:THEME.heading}}>{title}</h3>
      {subtitle && <p style={{margin:'4px 0 8px', color:THEME.textMuted, fontSize:12}}>{subtitle}</p>}
      {children}
    </div>
  );
}
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{width:'100%', height:260}}>
      {[0,0.25,0.5,0.75,1].map((t,i)=>{
        ...
        return (<g key={i}><line x1={pad} x2={W-pad} y1={gy} y2={gy} stroke={THEME.border}/><text x={8} y={gy+4} fontSize="11" fill="#374151">{gv}</text></g>);
      })}
      {/* x labels */}
      {data.map((d,i)=> i%Math.ceil(data.length/7)===0 &&
        <text key={i} x={pad+i*stepX} y={H-8} fontSize="10" textAnchor="middle" fill={THEME.textMuted}>{d.date.slice(5)}</text>
      )}
      ...
      {/* legend */}
      <g>
        <circle cx={pad} cy={16} r={4} fill={THEME.lineA}/><text x={pad+10} y={20} fontSize="11" fill="#1F2937">Scans</text>
        <circle cx={pad+70} cy={16} r={4} fill={THEME.lineB}/><text x={pad+80} y={20} fontSize="11" fill="#1F2937">Detected</text>
      </g>
    </svg>
  );
}
...
function Donut({ data, colors }:{data:{label:string;value:number}[]; colors:string[]}) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{width:160, height:160}}>
      ...
      <text x={cx} y={cy+4} textAnchor="middle" fontSize="11" fill={THEME.textMuted}>{total.toLocaleString()}</text>
    </svg>
  );
}
    ...
    <svg viewBox={`0 0 ${W} ${H}`} style={{width:'100%', height:200}}>
      ...
      {values.map((v,i)=>{
        ...
        return <g key={i}>
          ...
          <text x={x+barW/2} y={H-8} fontSize="10" textAnchor="middle" fill={THEME.textMuted}>{labels[i]}</text>
        </g>;
      })}
    </svg>
  );
...
function StatusBadge({status}:{status: CaseRow["status"]}) {
  ...
  const color = isFlagged ? THEME.primaryDark : THEME.red;
  const bg = isFlagged ? THEME.primarySoft : "#FEF2F2";
  return <span style={{
...
function Th({ children }:{children:React.ReactNode}) {
  return <th style={{textAlign:'left', padding:'8px', color:THEME.textMuted, fontWeight:600}}>{children}</th>;
}
